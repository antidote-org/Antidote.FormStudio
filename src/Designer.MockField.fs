...
let MockField (props: MockFieldProps) =
    let designerFieldOpt =
        props.ListOfFields |> List.tryFind (fun f -> f.Key = props.FormField.FieldKey)
        ...
        [
                        ...
                        prop.children
                            [
                                Bulma.messageBody
                                    [
                                        prop.text
                                            $"Field not found for key: %s{props.FormField.FieldKey}"
                                    ]
                            ]
                    ]
            ...
            | Some dep ->
                Bulma.icon
                    [
                        prop.style
                            [ style.floatStyle.right; style.bottom 20; style.position.relative ]
                        prop.children
                            [
                                Html.i
                                    [
                                        prop.style
                                            [
                                                if
                                                    System.String.IsNullOrWhiteSpace dep.FieldValue
                                                then
                                                    style.color "red"
                                                else
                                                    style.color "#26619b"
