...
let private classes : CssModules.FormWizard.Compose.FormComposeProgressWizard = import "default" "./FormComposeProgressWizard.module.scss"
...
let FormComposeProgressWizard (props: FormComposeProgressWizardProps) =
    Html.div [
        ...
        if props.FormTotalSteps > 1 then
            Html.ul [
                prop.classes ["steps"; "is-thin"; (if props.FormTotalSteps > 8 then "is-small" else "is-medium")]
                prop.style [
                ...
                prop.children [
                    for s in [1..props.FormTotalSteps] do
                        Html.li [
                            prop.classes ["steps-segment"; (if props.FormCurrentStep = s then "is-active" else "") ]
                            prop.children [
                                Html.a [
                                    prop.title (sprintf "Step %i" s)
                                    prop.style (if props.FormCurrentStep >= s then [] else [style.cursor.defaultCursor])
                                    prop.onClick (if props.FormCurrentStep >= s then (fun _ -> props.FormProgressStepAction s) else ignore)
                                    prop.classes ["steps-marker"; classes.stepMarker]
                                ]
                            ]
                        ]
                ]
