...
let private update (props: FormComposeProps) (msg : Msg) (model : FormComposeState) =
    ...
    | NavigateToStep newStep ->
        { model with CurrentStep = newStep },
        ...
        { model with
            ...
            DynamicForm =
                { model.DynamicForm with
                    Steps = model.DynamicForm.Steps.Change (
                        nextStepOrder,
...
let FormCompose (props: FormComposeProps)  =
    ...
    let progress =
        if state.ResultViewMode = FormComposeMode.ReadOnly then ReadOnly
        else
        ...
        | Some step ->
            Html.div [
                ...
                Html.div [
                    ...
                    prop.children [
                        step
                        |> Composer.compose (if state.ResultViewMode = FormComposeMode.ReadOnly then true else false)
                        |> Composer.render state.DynamicForm.Steps[StepOrder state.CurrentStep] dispatch (formAction progress (state.FormSaved && props.SubmissionSuccess ))
                    ]
                ]
            ]
