...
let StepBreak(props: StepBreakProps) =
    Html.div [
        prop.className [classes.stepBreak; if props.ActiveField.State = AddingDependantKeys then classes.disabled else "" ]
        prop.children [
            Html.button [
...
let LabelEdit(props:LabelEditProps) =
    if props.ActiveField.State = AddingDependantKeys then
        ...
        Html.input [
            if not (System.String.IsNullOrWhiteSpace(props.FormField.Label))
            then
            ...
            prop.onChange (fun (e:string) ->
                ...
                let outFormSpec =
                    props.FormSpec
                    |> updateFormFieldInFormSpecStep newFormField  props.FormStep
...
let FieldToolbar(props: FieldToolbarProps) =
    ...
    if props.ActiveField.State = AddingDependantKeys && props.FormField.FieldOrder < props.ActiveField.FormFieldNumber then
        Bulma.tag [
            Bulma.tag.isRounded
            prop.style [
                style.marginTop 5
            ...
            prop.children [
                Html.span [
                    prop.className "icon"
                    prop.children [
                        Html.i [ prop.className "fas fa-arrows-alt"]
                    ]
                ]
                Html.span [
                    prop.text "Drag Me!"
        ...
        if props.FormField.FieldOrder = props.ActiveField.FormFieldNumber then
            Bulma.buttons [
                ...
                prop.children [
                    Bulma.button.button [
                        ...
                        prop.onClick ( fun e ->
                            e.stopPropagation()
                            let step = props.FormSpec |> tryFindFormStepByStepNumber props.FormStepNumber
                            let outFormSpec = props.FormSpec |> moveFormFieldUpInFormSpec props.FormStepNumber props.FormField
                    ...
                    Bulma.button.button [
                        ...
                        prop.disabled (
                            let currentStep =
                                match step with
                                | Some step ->
                                    (step.Fields |> List.length) - 1
                                | None -> 0
                            props.FormField.FieldOrder = currentStep
                        )
                        prop.onClick ( fun e ->
                            e.stopPropagation()
                            let outFormSpec = props.FormSpec |> moveFormFieldDownInFormSpec props.FormStepNumber props.FormField
                            props.OnChange outFormSpec
                            props.SetActiveField {
                                props.ActiveField with
                                    FormFieldNumber = props.FormField.FieldOrder + 1
        ...
        prop.onDrop (fun (e: Types.DragEvent) ->
            ...
            | Some (DragSource.Designer_FormField_FieldKey fieldKey) ->
                ...
                let outFormSpec =
                    {
                        props.FormSpec with
                            Steps = props.FormSpec.Steps |> List.map (fun s ->
        ...
        prop.children [
            ...
            Html.div [
                ...
                prop.classes [
                    ...
                    if props.FormField.FieldOrder = props.ActiveField.FormFieldNumber && props.FormStep.StepOrder = props.SelectedStepNumber
                    then
                        classes.fieldSelected
                    else if props.ActiveField.State = AddingDependantKeys && props.FormField.FieldOrder < props.ActiveField.FormFieldNumber && props.FormStep.StepOrder = props.SelectedStepNumber then
                        classes.fieldDrag
                    else if props.ActiveField.State = AddingDependantKeys && props.FormField.FieldOrder > props.ActiveField.FormFieldNumber then //&& step.StepOrder < props.SelectedStepNumber then
                        classes.disabled
                    else if props.ActiveField.State = AddingDependantKeys && props.FormStep.StepOrder <> props.SelectedStepNumber then
                        classes.disabled
                ]
                ...
                prop.children [
                    ...
                    Antidote.FormDesigner.MockField.MockField {|
                        FormSpec = props.FormSpec
                        FormStep = props.FormStep
                        FormField = props.FormField
                        ActiveField = props.ActiveField
                        ...
                        Html.article [
                            ...
                            prop.onDrop (fun (e: Types.DragEvent) ->
                                ...
                                | Some (DragSource.Designer_FormFieldType_Key key) ->
                                    ...
                                    props.OnChange (addDesignerFieldTypeToStep key step.StepOrder props.FormSpec)
                                | _ -> ()
                            )
                            prop.children [
...
[<ReactComponent>]
let FormReview ((formSpec:FormSpec), (dynamicFormOpt: DynamicForm<Form.View.Model<DynamicStepValues>> option) )  =
    let flatDynamicFormOpt =
        dynamicFormOpt
        |> Option.map (fun df -> df |> Antidote.FormDesigner.Helper.flattenFormSteps)

    let flatSpec =  formSpec |> Helper.flattenSpecSteps

    match flatDynamicFormOpt with
    | None -> Html.none
    | Some flatDynamicForm ->
    Html.div [
        // prop.classes [ classes.pageContainer ]

        match formSpec.Steps |> List.tryFind (fun s -> s.StepOrder = 1) with
        | Some step ->
            prop.children [
                Html.div [
                    Bulma.text.div [
...
let DynamicFormPreview(props: DynamicFormPreviewProps) =
    ...
    Bulma.columns [
        ...
        Bulma.column [
            if showValuesReview then column.is6 else column.is12
            prop.children[
                Bulma.button.a [
                    color.isPrimary
...
let DynamicFormDesigner(props: FormStudioProps) =
    ...
    let selectedStepNumber, setSelectedStepNumber = React.useState (formSpec.Steps.[0].StepOrder)
