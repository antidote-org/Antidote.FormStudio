...
let CPTFinder (props : Field.ReactComponentField.ReactComponentFieldProps) =
    ...
    let validateCPT (cptCode: CPT) =
        if System.String.IsNullOrWhiteSpace cptCode.HCPCS
        then Error $"CPT HCPCS cannot be empty for ID: {cptCode.Id}"
        else Ok ()
        ...
        fun searchTerm ->
            ...
            then
                async {
                    ...
                    let getCPTRequest : Antidote.Core.V2.Domain.MedicalRegistry.Request.SearchMedicalRegistry =
                    ...
                    let! getCPTResults = Antidote.Client.API.EndPoints.medicalRegistry.SearchMedicalRegistry getCPTRequest
