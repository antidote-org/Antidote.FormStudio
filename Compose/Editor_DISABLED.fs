...
let private classes : CssModules.Compose.Editor = import "default" "./Editor.module.scss"
    ...
    let readFormSpecCommand =
        ...
        | Some formId, Some formVersion ->
            let unauthFormSpecRequest =
                { DynamicFormSpecId = Guid.Parse(formId); DynamicFormSpecVersion = Some formVersion } : Unauthenticated.Request.UnauthenticatedReadFormSpec
            Cmd.OfAsync.either
...
let update (msg:Msg) (state:State) =
    ...
    | Submit dynamicForm ->
        ...
        debuglog ( $"Editor: Update: Submit: DynamicFormResultData: {serializedDynamicFormResultData}" )
        state, Cmd.none
    ...
    | UpdateSpec specJson ->
        ...
        { state with Form = newForm; Review = false; SelectedFormSpecOpt = formSpecOpt }, Cmd.none

    | Review ->
        { state with Review = true }, Cmd.none
    ...
    | SelectFormSpec formSpec ->
        ...
        { state with Form = newForm; SelectedFormSpecOpt = Some formSpec ; Review = false }, Cmd.none
    ...
    | SaveFormSpec ->
        ...
        state
        , Cmd.OfAsync.either
            (Antidote.Client.User.UserSession.Instance.RemotingRequest Antidote.Client.API.EndPoints.form (fun services -> services.SaveFormSpec))
            request
            SaveFormSpecResult
            ErroredRequest
    ...
    | UnauthGotFormSpec (response: Unauthenticated.Response.UnauthenticatedReadFormSpec) ->
        ...
        | Unauthenticated.Response.UnauthenticatedReadFormSpec.Read formSpecResult ->
            ...
            { state with Form = newForm; Review = true; ViewMode = FillForm } , Cmd.none
    ...
    | ReadAllFormSpecs ->
        state
        , Cmd.OfAsync.either
            (Antidote.Client.User.UserSession.Instance.RemotingRequest Antidote.Client.API.EndPoints.form (fun services -> services.ReadAllFormSpecs))
    ...
    | ReadAllFormSpecsResult response ->
        ...
        | Antidote.Core.V2.Domain.Form.Response.ReadAllFormSpecs.ReadFormSpecs formSpecDtos ->
            let formSpecs =
                ...
                |> List.map (Migrator.FormSpec.decodeFormSpec Migrator.FormSpec.FormSpecInput.Unknown)
                            ...
                            prop.children [
                                ...
                                |> List.map (fun formSpec ->
                                    Html.option [
                                        ...
                                        prop.children [ Html.text formSpec.Title ]
                                    ]
                                ) |> React.fragment
                            ]
                        ]
...
[<ReactComponent>]
let SpecValuesEditor(state, dispatch, setDynamicForm, (dynamicForm: DynamicForm<Form.View.Model<DynamicStepValues>> option)) =
    Html.form [
        prop.style [ style.margin 20]
        prop.children [
            Bulma.field.div [
                ...
                Bulma.control.div [
                    Bulma.textarea [
                        prop.onChange (fun formValuesJson ->
                            let dynamicFormRes = deserializeForm state.SelectedFormSpecOpt formValuesJson
                            match dynamicFormRes with
                            | Ok dynamicForm ->
                                setDynamicForm (Some dynamicForm)

                            | Result.Error err ->
                                toast (Html.div "Error decoding form values from string!" ) |> ignore
...
let Editor() =
    ...
    let dynamicFormOpt, setDynamicForm = React.useState<DynamicForm<Form.View.Model<DynamicStepValues>> option> (None)
        ...
        | true ->
            ...
            | Some formSpec ->
                Bulma.columns [
                    ...
                    Bulma.column [
                        DynamicFormRenderer ( formSpec, dispatch, FormComposeMode.Editable, setDynamicForm, dynamicFormOpt )
                    ]
                    Bulma.column [
                        match state.ViewMode with
                        | Editor ->
