...
let allStepsHaveFields (formSteps: FormStep list) =
    formSteps
    |> List.forall (fun s -> s.Fields |> List.isEmpty |> not)

let totalNumberOfFieldsInSpec formSpec =
    formSpec.Steps
    |> List.sumBy (fun step -> step.Fields |> List.length)
...
let saveFormSpec (formSpec: Antidote.Core.FormProcessor.Spec.v2_0_1.FormSpec) (metaData: FormMetaData) (setSpecFun: Antidote.Core.FormProcessor.Spec.v2_0_1.FormSpec -> unit) =
    async {
        let formSpecId =
            if formSpec.Id = Guid.Empty
            then Guid.NewGuid()
            else formSpec.Id
...
let getDesignerFieldType fieldName =
    match fieldName with
    | "Image" -> DesignerFieldType.Image
    | "Message" -> DesignerFieldType.Message
    | "Signature" -> DesignerFieldType.Signature
    | "Single Choice" -> DesignerFieldType.SingleChoice
    | "Multi Choice" -> DesignerFieldType.MultiChoice
    | "Text" -> DesignerFieldType.Text
    | "Text Area" -> DesignerFieldType.TextArea
    | "Tel" -> DesignerFieldType.Tel
    | "Date" -> DesignerFieldType.Date
    | "Time" -> DesignerFieldType.Time
    | "Number" -> DesignerFieldType.Number
    | "USA State Selector" -> DesignerFieldType.StateSelectorUSA
    | "Yes / No" -> DesignerFieldType.YesNo
    | "True / False" -> DesignerFieldType.TrueFalse
    | "Radio" -> DesignerFieldType.Radio
    | "Checkbox" -> DesignerFieldType.Checkbox
    | "Checkbox List" -> DesignerFieldType.CheckboxList
    | "Dropdown" -> DesignerFieldType.Dropdown
    | "Tag List" -> DesignerFieldType.TagList
    | "Switch" -> DesignerFieldType.Switch
    | "Text Auto Complete" -> DesignerFieldType.TextAutoComplete
    | "ePrescribe" -> DesignerFieldType.EPrescribe
    | "Speech To Text" -> DesignerFieldType.SpeechToText
    | "Drug Finder" -> DesignerFieldType.DrugFinder
    | "Drug Finder With Frequency" -> DesignerFieldType.DrugFinderWithFrequency
    // TODO: Unify these controls after extending the
    // FormSpec to include properties for determining the appropriate 'finder'
    | "Allergy Finder" -> DesignerFieldType.AllergyFinder
    | "CPT Finder" -> DesignerFieldType.CPTFinder
    | "ICD10 Finder" -> DesignerFieldType.ICD10Finder
    | _ -> failwithf "Unknown field type: %s" fieldName
...
let defaultField (designerFieldType:DesignerFieldType) fieldOrder fieldType =
    {
        FieldType = fieldType
...
let deleteOptionInFormField field (option:Spec.v2_0_1.FieldOption) =
    match field.FieldType with
    | Dropdown f -> { field with FieldType = Dropdown { f with Options = f.Options |> List.filter (fun o -> o.OptionKey <> option.OptionKey ) } }
    | TagList f -> { field with FieldType = TagList { f with Options = f.Options |> List.filter (fun o -> o.OptionKey <> option.OptionKey ) } }
    | Radio f -> { field with FieldType = Radio { f with Options = f.Options |> List.filter (fun o -> o.OptionKey <> option.OptionKey ) } }
    | CheckboxList f -> { field with FieldType = CheckboxList { f with Options = f.Options |> List.filter (fun o -> o.OptionKey <> option.OptionKey ) } }
    | MultiChoice f -> { field with FieldType = MultiChoice { f with Options = f.Options |> List.filter (fun o -> o.OptionKey <> option.OptionKey ) } }
    | SingleChoice f -> { field with FieldType = SingleChoice { f with Options = f.Options |> List.filter (fun o -> o.OptionKey <> option.OptionKey ) } }
    | TextAutoComplete f -> { field with FieldType = TextAutoComplete { f with Options = f.Options |> List.filter (fun o -> o.OptionKey <> option.OptionKey ) } }
...
let createOption (description: string) (value:string) = {
    ...
    Value = if value = "" then description else value
    OptionKey = System.Guid.NewGuid().ToString()
}

let updateOptionInField (field:FormField) (option: FieldOption) =
    match field.FieldType with
    | Dropdown f -> { field with FieldType = Dropdown { f with Options = f.Options |> List.map (fun o -> if o.OptionKey = option.OptionKey then option else o) } }
    | TagList f -> { field with FieldType = TagList { f with Options = f.Options |> List.map (fun o -> if o.OptionKey = option.OptionKey then option else o) } }
    | Radio f -> { field with FieldType = Radio { f with Options = f.Options |> List.map (fun o -> if o.OptionKey = option.OptionKey then option else o) } }
    | CheckboxList f -> { field with FieldType = CheckboxList { f with Options = f.Options |> List.map (fun o -> if o.OptionKey = option.OptionKey then option else o) } }
    | MultiChoice f -> { field with FieldType = MultiChoice { f with Options = f.Options |> List.map (fun o -> if o.OptionKey = option.OptionKey then option else o) } }
    | SingleChoice f -> { field with FieldType = SingleChoice { f with Options = f.Options |> List.map (fun o -> if o.OptionKey = option.OptionKey then option else o) } }
    | TextAutoComplete f -> { field with FieldType = TextAutoComplete { f with Options = f.Options |> List.map (fun o -> if o.OptionKey = option.OptionKey then option else o) } }
...
let createFormFieldWithOptions field options =
    ...
    | CheckboxList f -> { field with FieldType = CheckboxList { f with Options = f.Options @ options } }
    | MultiChoice f -> { field with FieldType = MultiChoice { f with Options = f.Options @ options } }
    | SingleChoice f -> { field with FieldType = SingleChoice { f with Options = f.Options @ options } }
    | TextAutoComplete f -> { field with FieldType = TextAutoComplete { f with Options = f.Options @ options } }
...
let updateFormFieldInFormSpecStep (inField:FormField) (formStep:FormStep) (formSpec:FormSpec) : FormSpec =
    let outStep = {
        formStep with
            Fields =
                formStep.Fields
                |> List.map (fun f ->
                    if f.FieldKey = inField.FieldKey
                    then inField
                    else f
                )
    }
...
let addFormFieldToStep (formStep:FormStep) (formSpec:FormSpec) (formField:FormField) : FormSpec =
    ...
    let outStep = {
        formStep with
            Fields = formStep.Fields @ [ formField ]
    }
...
let insertFormFieldToStepAt (formStep:FormStep) newPositionFieldOrder fieldKey =
    formStep.Fields
    |> List.insertAt (newPositionFieldOrder - 1) (formStep.Fields |> List.find (fun f -> f.FieldKey = fieldKey))
...
let tryFindFormStepByStepNumber (stepNumber:int) (formSpec:FormSpec) : FormStep option =
    formSpec.Steps
    |> List.tryFind (fun step -> step.StepOrder = stepNumber)
...
let moveFieldByKeyToPositionInFormStepSpec fieldKey newPositionFieldOrder (formStepOrder) (formSpec:FormSpec) =
                ...
                |> List.map (fun s ->
                    if s.StepOrder = formStep.StepOrder then
                        { s with
                            Fields =
                                s.Fields
                                |> List.insertAt (newPositionFieldOrder - 1) (s.Fields |> List.find (fun f -> f.FieldKey = fieldKey))
                                |> List.distinct
                                |> List.mapi (fun index f -> { f with FieldOrder = index + 1 } )
...
let tryGetStepByNumber (stepNumber:int) (formSpec:FormSpec) =
    formSpec.Steps
    |> List.tryFind (fun s -> s.StepOrder = stepNumber)
...
let tryFindFieldInSpec (fieldNumber:int) (stepNumber:int) (formSpec:FormSpec) : FormField option =
    let stepOpt =
        formSpec.Steps
        |> List.tryFind (fun step -> step.StepOrder = stepNumber)
    match stepOpt with
    ...
    | Some step ->
        let fieldOpt =
            step.Fields
            |> List.tryFind (fun field -> field.FieldOrder = fieldNumber)
        match fieldOpt with
        | None -> None
        | Some field -> Some field

let getDefaultFormFieldByDesignerFieldType fieldOrder (fieldType:DesignerFieldType) =
    let outField =
...
let insertDesignerFieldTypeToStepAt (source:string) (stepNumber:int) (at:int) (formSpec:FormSpec) : FormSpec =
    let step =
        formSpec.Steps
        |> List.find (fun step -> step.StepOrder = stepNumber)

    let ff =
        getDesignerFieldType source
...
let addDesignerFieldTypeToStep (source:string) (stepNumber:int) (formSpec:FormSpec) : FormSpec =
...
let flattenSpecSteps formSpec: FormSpec =
    let rec flatten (steps: FormStep list) (acc: FormField list) =
        ...
        | step :: rest ->
            let reorderedFields =
                step.Fields
                |> List.map (fun field -> { field with FieldOrder = step.StepOrder * 1000 + field.FieldOrder } )
...
let flattenFormSteps (dynamicForm:  DynamicForm<Form.View.Model<DynamicStepValues>>) : DynamicForm<Form.View.Model<DynamicStepValues>> =
    let flatForm =
        dynamicForm.Steps
        |> Map.fold (fun acc k v ->
            (v.Values |> Map.toList) @ acc
        ) []
        |> Map.ofList
        |> Form.View.idle
...
let extractDataFromFableFormsModel (dynamicForm:DynamicForm<Form.View.Model<DynamicStepValues>>) : DynamicFormResultData =
    {
        ResultFormSpecDetails = dynamicForm.DynamicFormSpecDetails
        ResultSteps =
            dynamicForm.Steps
            |> Map.map (fun key dynamicStepValues ->
                dynamicStepValues.Values
            )
    }