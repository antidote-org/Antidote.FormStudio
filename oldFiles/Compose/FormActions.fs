    ...
    (dispatch : Dispatch<Msg>)
     =
        ...
        Bulma.field.div [
            ...
            prop.children [
                Bulma.control.div [
                    prop.style [ style.custom ("minWidth","50%")]
                    prop.children [
                        Bulma.button.a [
                            button.isOutlined
                            prop.style [ style.custom ("minWidth","100%")]
...
let formAction stepProgress isSubmitted =
    ...
    | Middle -> Form.View.Action.Custom (previousAndNextAction (t Intl.Previous.Key) (t Intl.Next.Key))
    | Last ->
        if isSubmitted then
            Form.View.Action.Custom (previousAction (t Intl.Previous.Key))
        else
            Form.View.Action.Custom (previousAndSubmitAction (t Intl.Previous.Key) (t Intl.Submit.Key))
