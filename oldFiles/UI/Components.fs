...
let private classes : CssModules.UI.Default = import "default" "./Default.module.scss"
...
module CAGE =
    ...
    let screeningVerbiage score : ResizeArray<string> =
        let screeningVerbiage:ResizeArray<string> =
            ...
            | 0
            | 1 ->
                ResizeArray ["Screening Negative"]
            | 2
            | 3
            | 4 ->
                ResizeArray ["Scores of 2 or higher had a 93% sensitivity / 76% specificity for the identification of “excessive drinking” and a 91% sensitivity / 77% specificity for the identification of alcoholism."]
            | _ -> ResizeArray[]
            ...
            prop.children[
                ...
                if (flags |> Set.contains "EyeOpener") then
                    Html.div [
                        ...
                        prop.children [
                            Html.div [ Html.span "Some clinicians also consider the eye opener question as highly concerning for unhealthy drinking behavior, even if all other questions are answered negatively." ]
                            Html.br []
                        ]
                    ]
                else
            ...
            prop.children[
                ...
                if suicidal then
                    Html.div [
                        prop.style [style.color "#ceff00";]
                        prop.children [
                            Html.div [ Html.span (t Intl.SuicidalWarning.Key)]
                            Html.br []
                        ]
                    ]
                else
            ...
            prop.children[
                ...
                if ((flags |> Set.count) > 0) then
                    Html.div [
                        prop.style [style.color "#ceff00";]
                        prop.children [
                            Html.div [ Html.span "WARNING: Patient is at risk for substance use disorder. Substance use disorders should be evaluated by a psychiatrist, psychologist, or licensed counselor specializing in drug and alcohol addictions."]
                            Html.br []
                        ]
                    ]
                else
    ...
    let clinicalNote ( note: FieldResult ) =
        Html.p [
            ...
            prop.children [
                ...
                Html.strong [
                    color.hasTextInfo
                    prop.text note.Patient
                ]
                Html.span " was asked: "
                Html.strong [
                    color.hasTextSuccessDark
                    prop.text note.Question
                ]
                // Html.br []
                Html.span " The patient reported: "
                Html.strong [
                    color.hasTextDangerDark
                    prop.text note.Answer
                ]
            ]
        ]
            ...
            prop.children[
                ...
                |> Seq.map (fun i ->
                    ...
                    | RenderingMode.Normal str ->
                        Html.div [
                            prop.style [style.color "#FFF"; style.display.block]
                            prop.text str
                        ]

                    | RenderingMode.Warning str ->
                        Html.span [
                            prop.style [style.color "#ceff00"; style.display.block ]
                            prop.children [
                                Html.span [ Html.span str ]
                                Html.br []
                            ]
                        ]
